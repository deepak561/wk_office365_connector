...
month_name = {"January": 'Jan', "February": 'Feb', "March":'Mar',"April":'Apr',"May":'May',"June":'Jun',"July":'Jul',"August":'Aug',"September":'Sep',"October":'Oct',
	"November":'Nov',"December":'Dec'}

month_name1 = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']

class Office365Main(http.Controller):

	@http.route('/office365_odoo_connector/',type='http',auth='user')
	def office365_odoo_connector(self, *args,**kwargs):
		...
		try:
			...
			action_id = request.env.ref('office365_odoo_connector.office365_connection_mapping').id
	...
	@http.route('/office365_odoo_connector/fetch_instace_id',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/change_instance_id',type='json',auth='user')
	def change_instance_id(self,instance_id=1):
		request.session['instance_id'] = int(instance_id)
		return {'instance_id':int(instance_id)}
	

	@http.route('/office365_odoo_connector/fetch_instace_details',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/fetch_instance_extra_details',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/get_synchronisation_id', type ='json', auth='user')
	...
	@http.route('/office365_odoo_connector/fetch_calendar_events_details',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/calender_month_detail',type='json',auth='user')
	def calender_month_detail(self, instance_id=1,created_by='export',month=False):
		...
		# else:
		# 	month = current_date.split(' ')[1]
		# 	month = month_name1[int(month)-1]
		# 	select_sql_clause = """select name,start from calendar_event where id in 
		# 	(select name from office365_calendar_mapping where instance_id=%d and created_by=%r) and to_char(start,'Mon') ='%s'"""%(int(instance_id),str(created_by),str(month))
		request.env.cr.execute(select_sql_clause)
		query_results = request.env.cr.dictfetchall()
		_logger.info("==================calender_month_detail====================query_results%r",query_results)
		# _logger.info("==================calender_month_detail====================month1%r",datetime.month())
		# res = []
		for result in query_results:
			dt = result['start']
	...
	@http.route('/office365_odoo_connector/fetch_project_details',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/fetch_task_doughnut_data',type='json',auth='user')
	...
	@http.route('/office365_odoo_connector/get_dashboard_line_data',type = 'json', auth = 'user')
